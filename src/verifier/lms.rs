...
// Single-tree HSS, SHA-256, H=5 (32 leaves), W=8 (smallest signature).
// HSS_LEVELS=1 keeps the row a "plain LMS" data point; keygen is fast
// enough that the bench can mint a fresh key per signer per run. LMS is
// stateful — never reuse a signing key across runs in production.
